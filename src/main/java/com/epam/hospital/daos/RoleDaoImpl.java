...
public class RoleDaoImpl implements RoleDao {
    ...
    public void save(Role role) {
        try (Connection connection = DataSource.getConnection();
            ...
            if (isExecuted) log.info("New role is added: {}", role.getName());
        } catch (SQLException e) {
            log.error(e.getMessage());
            throw new RuntimeException(e);
        }
    ...
    public void deleteById(long id) {
        try (Connection connection = DataSource.getConnection();
            ...
            preparedStatement.setLong(1, id);
            boolean isExecuted = preparedStatement.execute();
            if (isExecuted) log.info("Role with ID {} is deleted", id);
        } catch (SQLException e) {
            log.error(e.getMessage());
            throw new RuntimeException(e);
        }
    ...
    public void update(Role role) {
        try (Connection connection = DataSource.getConnection();
            ...
            boolean isExecuted = preparedStatement.execute();
            if (isExecuted) log.info("Role with ID {} is updated", role.getId());
        } catch (SQLException e) {
            log.error(e.getMessage());
            throw new RuntimeException(e);
        }
