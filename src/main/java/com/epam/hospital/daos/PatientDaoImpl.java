...
public class PatientDaoImpl implements PatientDao {
    ...
    public void save(Patient patient) {
        try (Connection connection = DataSource.getConnection();
            ...
            if (isExecuted) log.info("New patient is added: {}", patient.getUsername());
        } catch (SQLException e) {
            log.error(e.getMessage());
            throw new RuntimeException(e);
        }
    ...
    public void deleteById(long id) {
        try (Connection connection = DataSource.getConnection();
            ...
            preparedStatement.setLong(1, id);
            boolean isExecuted = preparedStatement.execute();
            if (isExecuted) log.info("Patient with ID {} is deleted", id);
        } catch (SQLException e) {
            log.error(e.getMessage());
            throw new RuntimeException(e);
        }
    ...
    public void update(Patient patient) {
        try (Connection connection = DataSource.getConnection();
            ...
            boolean isExecuted = preparedStatement.execute();
            if (isExecuted) log.info("Patient is updated: {}", patient.getUsername());
        } catch (SQLException e) {
            log.error(e.getMessage());
            throw new RuntimeException(e);
        }
    ...
    public List<Patient> findAllPageableAndSorted(int offset, int numberOfRows, Sorting sortBy) {
        String query = "select * from patients order by " + sortBy.getValue() + " limit ? offset ?";
        List<Patient> patients = new ArrayList<>();
        try (Connection connection = DataSource.getConnection();
