...
public class HospitalStaffDaoImpl implements HospitalStaffDao {
    ...
    private static final String FIND_ALL_PAGEABLE_AND_SORTED_BY_CATEGORY = "select * from hospital_staff join " +
            "categories c on hospital_staff.category_id = c.id order by c.name limit ? offset ?";
    ...
    public Optional<HospitalStaff> findById(long id) {
        try (Connection connection = DataSource.getConnection();
            ...
            if (resultSet.next()) {
                ...
                doctor.setRole(new Role(resultSet.getLong(++columnIndex)));
                doctor.setCategory(new Category(resultSet.getLong(++columnIndex)));
    ...
    public void save(HospitalStaff hospitalStaff) {
        try (Connection connection = DataSource.getConnection();
            ...
            log.info("New hospital staff is added: {}", hospitalStaff.getUsername());
        } catch (SQLException e) {
            log.error(e.getMessage());
            throw new RuntimeException(e);
        }
    ...
    public void deleteById(long id) {
        try (Connection connection = DataSource.getConnection();
            ...
            preparedStatement.setLong(1, id);
            preparedStatement.execute();
            log.info("New hospital staff with ID {} is deleted", id);
        } catch (SQLException e) {
            log.error(e.getMessage());
            throw new RuntimeException(e);
        }
    ...
    public void update(HospitalStaff hospitalStaff) {
        try (Connection connection = DataSource.getConnection();
            ...
            preparedStatement.execute();
            log.info("Hospital staff is updated: {}", hospitalStaff.getUsername());
        } catch (SQLException e) {
            log.error(e.getMessage());
            throw new RuntimeException(e);
        }
    ...
    public List<HospitalStaff> findAllPageableAndSortedByCategory(int offset, int numberOfRows) {
        List<HospitalStaff> doctors = new ArrayList<>();
        try (Connection connection = DataSource.getConnection();
             PreparedStatement preparedStatement = connection.prepareStatement(FIND_ALL_PAGEABLE_AND_SORTED_BY_CATEGORY)) {
            preparedStatement.setInt(1, numberOfRows);
            preparedStatement.setInt(2, offset);
            ResultSet resultSet = preparedStatement.executeQuery();
            while (resultSet.next()) {
    ...
    public List<HospitalStaff> findAllByPatientId(long id) {
        ...
        try (Connection connection = DataSource.getConnection();
            ...
            while (resultSet.next()) {
                ...
                doctor.setRole(new Role(resultSet.getLong("role_id")));
                doctor.setCategory(new Category(resultSet.getLong("category_id")));
