...
public class HospitalCardRepositoryImpl implements HospitalCardRepository {
    ...
    public Optional<HospitalCard> getHospitalCardById(long id) {
        if (hospitalCardDao.findById(id).isPresent()) {
            ...
            hospitalCard.setPatient(patientRepository.getPatientById(hospitalCard.getPatient().getId()).orElseThrow(IllegalArgumentException::new));
            hospitalCard.setDoctor(hospitalStaffRepository.getHospitalStaffById(hospitalCard.getDoctor().getId()).orElseThrow(IllegalArgumentException::new));
