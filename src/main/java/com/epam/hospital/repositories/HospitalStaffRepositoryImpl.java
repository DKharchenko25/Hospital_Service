...
public class HospitalStaffRepositoryImpl implements HospitalStaffRepository {
    ...
    public void addStaff(HospitalStaff hospitalStaff) {
        hospitalStaff.setRole(roleDao.findByName(hospitalStaff.getRoleName()).orElseThrow(IllegalArgumentException::new));
        hospitalStaff.setCategory(categoryDao.findByName(hospitalStaff.getCategoryName()).orElseThrow(IllegalArgumentException::new));
    ...
    public Optional<HospitalStaff> getHospitalStaffById(long id) {
        if (hospitalStaffDao.findById(id).isPresent()) {
            ...
            hospitalStaff.setRole(roleDao.findById(hospitalStaff.getRoleId()).orElseThrow(IllegalArgumentException::new));
            hospitalStaff.setCategory(categoryDao.findById(hospitalStaff.getCategoryId()).orElseThrow(IllegalArgumentException::new));
    ...
    public List<HospitalStaff> getAllHospitalStaffPageableAndSortedByCategory(int offset, int numberOfRows) {
        List<HospitalStaff> hospitalStaffList = hospitalStaffDao.findAllPageableAndSorted(offset, numberOfRows, Sorting.BY_CATEGORY);
        hospitalStaffList.forEach(hospitalStaff -> {
            hospitalStaff.setRole(roleDao.findById(hospitalStaff.getRoleId()).orElseThrow(IllegalArgumentException::new));
            hospitalStaff.setCategory(categoryDao.findById(hospitalStaff.getCategoryId()).orElseThrow(IllegalArgumentException::new));
        });
