...
<div class="card bg-light mb-3 border-info" style="width: 40rem;">
    ...
    <form class="form-control" action="${pageContext.request.contextPath}/appoint_patient" method="post">
        ...
        <select name="doctorId" class="form-select" aria-label="Hospital staff">
            <c:forEach var="doctor" items="${requestScope.doctors}">
                <option value="${doctor.id}">${doctor.firstName} ${doctor.lastName} (${doctor.category})</option>
            </c:forEach>
        </select>
        <div class="card-body">
